...
function(BII_GENERATE_EXECUTABLE USER BLOCK FNAME)
    ...
    if(BII_${FNAME}_DEPS)
        SET (DEPS_LIB ${BII_${FNAME}_DEPS} "${USER}_${BLOCK}")
    else()
        set(DEPS_LIB "${USER}_${BLOCK}")
    endif()

    IF(DEPS_LIB )
        BII_GENERATE_ARDUINO_FIRMWARE("${vname}" "${aux_src}" "${DEPS_LIB}")
        add_dependencies(${vname} ${DEPS_LIB})
    else()
        BII_GENERATE_ARDUINO_FIRMWARE("${vname}" "${aux_src}" "")
    endif()
...
function(BII_GENERATE_LIB USER BLOCK)
    ...
    if(aux_deps)
        if(NOT BII_LIB_SRC)
            ...
            TARGET_LINK_LIBRARIES( ${vname} INTERFACE ${aux_deps})
        else()
            BII_GENERATE_ARDUINO_LIB("${vname}" "${aux_src}" "${aux_deps}")
        ...
        if(NOT BII_LIB_SRC)
            ...
            BII_GENERATE_ARDUINO_LIB("${vname}" "${aux_src}" "")
        endif()
    endif()
...
#=============================================================================#
#
#                        ARDUINO MACRO SECTION
#
#=============================================================================#

macro(BII_GENERATE_ARDUINO_LIB TARGET_LIB INPUT_SRCS DEP_LIBS)
    # if (not INPUT_SRCS)
    #     message(FATAL_ERROR  "There is not any files: ${INPUT_SRCS}" )
    # endif()
    if(NOT "${DEP_LIBS}" STREQUAL "")
        generate_arduino_library(${TARGET_LIB}
            SRCS ${INPUT_SRCS}
            LIBS ${DEP_LIBS}
        )
    else()
...
endmacro(BII_GENERATE_ARDUINO_LIB)


macro(BII_GENERATE_ARDUINO_FIRMWARE TARGET_FIRMWARE INPUT_SRCS DEP_LIBS)
    # if (not INPUT_SRCS)
    #     message(FATAL_ERROR  "There is not any files: ${INPUT_SRCS}" )
    # endif()
    if(NOT "${DEP_LIBS}" STREQUAL "")
        generate_arduino_firmware(${TARGET_FIRMWARE}
            SRCS ${INPUT_SRCS}
            LIBS ${DEP_LIBS}
        )
    else()
...
endmacro(BII_GENERATE_ARDUINO_FIRMWARE)
