#=============================================================================#
#                                                                             #
#            ARDUINO OPTIONS CONFIGURATION (PORT, PROGRAMMER, SERIAL, etc)    #
#                                                                             #
#=============================================================================#
...
function(GENERATE_ARDUINO_LIBRARY INPUT_NAME)
    ...
    find_arduino_libraries(TARGET_LIBS "${ALL_SRCS}" "" False)
...
function(GENERATE_ARDUINO_FIRMWARE INPUT_NAME)
    ...
    find_arduino_libraries(TARGET_LIBS "${ALL_SRCS}" "${INPUT_ARDLIBS}" False)
    foreach(LIB_DEP ${TARGET_LIBS})
...
function(get_arduino_flags COMPILE_FLAGS_VAR LINK_FLAGS_VAR BOARD_ID MANUAL)
    ...
    if(BOARD_CORE)
        ...
        if(NOT MANUAL)
            ...
            if(ARDUINO_SDK_VERSION VERSION_GREATER 1.5 OR ARDUINO_SDK_VERSION VERSION_EQUAL 1.5)
                set(COMPILE_FLAGS "${COMPILE_FLAGS} -I\"${ARDUINO_AVR_LIBRARIES_PATH}\"")
            endif()
        endif()
		...
		if(ARDUINO_SDK_VERSION VERSION_GREATER 1.5 OR ARDUINO_SDK_VERSION VERSION_EQUAL 1.5)
			set(COMPILE_FLAGS "${COMPILE_FLAGS} -DARDUINO_ARCH_AVR=1")
        endif()
...
#
#=============================================================================#
function(find_arduino_libraries VAR_NAME SRCS ARDLIBS SEARCH_PARENT)
	if(ARDUINO_LIBS)
		set(ARDUINO_LIBS ${ARDUINO_LIBS})
	else()
		set(ARDUINO_LIBS )
	endif()
    get_arduino_lib_folders(ARDUINO_LIBS_FOLDERS ${ARDUINO_LIBRARIES_PATH})
	if(ARDUINO_SDK_VERSION VERSION_GREATER  1.5)
		get_arduino_lib_folders(ARDUINO_1_5_LIBS_FOLDERS ${ARDUINO_AVR_LIBRARIES_PATH})
		foreach(LIB_1_5 ${ARDUINO_1_5_LIBS_FOLDERS})
			list(APPEND ARDUINO_LIBS_FOLDERS ${LIB_1_5})
		endforeach()
	endif()
    foreach(SRC ${SRCS})
        ...
        if(NOT ${_srcfile_generated} OR ${_sketch_generated})
            ...
            foreach(SRC_LINE ${SRC_CONTENTS})
                if("${SRC_LINE}" MATCHES "^[ \t]*#[ \t]*include[ \t]*[<\"]([^>\"]*)[>\"]")
					...
					list(APPEND LIB_SEARCH_PATHS ${LIBRARY_SEARCH_PATH} ${ARDUINO_LIBRARIES_PATH} ${CMAKE_CURRENT_SOURCE_DIR} ${CMAKE_CURRENT_SOURCE_DIR}/libraries ${ARDUINO_EXTRA_LIBRARIES_PATH})
					if(ARDUINO_SDK_VERSION VERSION_GREATER  1.5)
						list(APPEND LIB_SEARCH_PATHS ${ARDUINO_AVR_LIBRARIES_PATH})
					endif()
                    ...
                    endforeach()
                    if (${SEARCH_PARENT})
                        check_is_not_library(IS_NOT_LIBRARY "${ARDUINO_LIBS_FOLDERS}" "${INCLUDE_NAME}")
                        if (${IS_NOT_LIBRARY})
                            foreach(LIB_FOLDER ${ARDUINO_LIBS_FOLDERS})
                                if(EXISTS ${ARDUINO_LIBRARIES_PATH}/${LIB_FOLDER}/${CMAKE_MATCH_1})
                                    list(APPEND ARDUINO_LIBS ${ARDUINO_LIBRARIES_PATH}/${LIB_FOLDER})
                                    break()
                                elseif(EXISTS ${ARDUINO_LIBRARIES_PATH}/${LIB_FOLDER}/src/${CMAKE_MATCH_1})
                                    list(APPEND ARDUINO_LIBS ${ARDUINO_LIBRARIES_PATH}/${LIB_FOLDER}/src)
                                    break()
								elseif(ARDUINO_SDK_VERSION VERSION_GREATER  1.5)
									if(EXISTS ${ARDUINO_AVR_LIBRARIES_PATH}/${LIB_FOLDER}/${CMAKE_MATCH_1})
										list(APPEND ARDUINO_LIBS ${ARDUINO_AVR_LIBRARIES_PATH}/${LIB_FOLDER})
										break()
									elseif(EXISTS ${ARDUINO_AVR_LIBRARIES_PATH}/${LIB_FOLDER}/src/${CMAKE_MATCH_1})
										list(APPEND ARDUINO_LIBS ${ARDUINO_AVR_LIBRARIES_PATH}/${LIB_FOLDER}/src)
										break()
									endif()
                                endif()
                            endforeach()
...
# [PRIVATE/INTERNAL]
#
# get_arduino_lib_folders(VAR_NAME LIB_PATH)
#
#        VAR_NAME    - Variable wich will hold the generated library name folders
#        LIB_PATH    - Path to the SDK libraries path
#
#=============================================================================#
MACRO(get_arduino_lib_folders VAR_NAME LIB_PATH)
...
# [PRIVATE/INTERNAL]
#
# check_is_not_library(VAR_NAME  SUB_FOLDERS INCLUDE_NAME)
#
#        VAR_NAME       - Variable wich will hold the generated library name folders
#        SUB_FOLDERS    - All libraries folders
#        INCLUDE_NAME   - Include file name (without extension)
#
#=============================================================================#
function(check_is_not_library VAR_NAME SUB_FOLDERS INCLUDE_NAME)
    set(${VAR_NAME} True PARENT_SCOPE)
    foreach (folder ${SUB_FOLDERS})
        if (${folder} STREQUAL ${INCLUDE_NAME})
...
function(setup_arduino_library VAR_NAME BOARD_ID LIB_PATH COMPILE_FLAGS LINK_FLAGS)
    set(LIB_TARGETS)
    set(LIB_INCLUDES)
    string(REGEX REPLACE "/src" "" CLEAN_LIB_PATH ${LIB_PATH})
    get_filename_component(LIB_NAME ${CLEAN_LIB_PATH} NAME)
    ...
    if(NOT TARGET ${TARGET_LIB_NAME})
        ...
        if(LIB_SRCS)
            ...
            get_arduino_flags(ARDUINO_COMPILE_FLAGS ARDUINO_LINK_FLAGS ${BOARD_ID} FALSE)
            find_arduino_libraries(LIB_DEPS "${LIB_SRCS}" "" False)
...
function(setup_arduino_libraries VAR_NAME BOARD_ID SRCS ARDLIBS COMPILE_FLAGS LINK_FLAGS)
    set(LIB_TARGETS)
    set(LIB_INCLUDES)
    find_arduino_libraries(TARGET_LIBS "${SRCS}" ARDLIBS True)
...
function(setup_arduino_target TARGET_NAME BOARD_ID ALL_SRCS ALL_LIBS COMPILE_FLAGS LINK_FLAGS MANUAL)
    ...
    set(PATH_TO_SAVE ${CMAKE_HOME_DIRECTORY}/../bin)
    set(TARGET_PATH ${PATH_TO_SAVE}/${TARGET_NAME})
...
if(NOT ARDUINO_FOUND AND ARDUINO_SDK_PATH)
	...
	if(ARDUINO_SDK_VERSION VERSION_GREATER  1.5)
		...
		find_file(ARDUINO_AVR_LIBRARIES_PATH
			NAMES /hardware/arduino/avr/libraries
			PATHS ${ARDUINO_SDK_PATH}
			DOC "Path to directory containing the Arduino libraries.")
	else()
